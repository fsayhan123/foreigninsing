        ...
        <div v-else> 
            ...
            <div class = "SignUpContainer">
                ...
                <b-form class = "mt-4 formContainer" v-on:submit.prevent = "updateProfile">
                    <img :src = "previewImage" style = "width:300px; height:300px; object-fit: scale-down; border-radius:25%;"/> 
                    <br />
                    <b-form-file id = "profilePicture" accept = "image/*" v-on:change = "previewImageFunc"></b-form-file>
                    <b-form-input placeholder = "Username" class = "mt-4" v-model = "userName" required></b-form-input>
...
export default {
    ...
    methods : {
        updateProfile : async function() {
            if (this.previewImage === null) {
                ...
                var profileRef = firebase.storage().ref().child(`profilePictures/${this.uid}.jpg`);
    ...
    mounted() {
        firebase.auth().onAuthStateChanged(async user => {
            if (user) {
                this.uid = user.uid;
                await database.collection("users").doc(user.uid)
                    .get()
                    .then((doc) => {
                        this.user = doc.data()
